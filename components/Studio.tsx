    ...
    <section id="studio" className="relative py-28 md:py-40">
      <div className="container-page grid gap-16 lg:grid-cols-[1fr_1.4fr] lg:gap-24">
        <div className="lg:sticky lg:top-32 lg:self-start">
          <Reveal>
            <p className="eyebrow">
              <span className="h-px w-6 bg-gold-300/60" /> The Studio
            </p>
        ...
        <div className="grid gap-px overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.03] md:grid-cols-2">
          {principles.map((p, i) => (
            <Reveal
              key={p.n}
              delay={i * 0.08}
              className="bg-ink-900/40 p-8 md:p-10"
            >
              <div className="font-mono text-xs uppercase tracking-[0.22em] text-gold-300">
                {p.n}
              </div>
              <h3 className="mt-4 font-display text-2xl text-slate-50">
                {p.title}
              </h3>
